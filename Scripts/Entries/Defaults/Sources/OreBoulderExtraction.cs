...
namespace ItemBrowser.Entries.Defaults.Sources {
	public class OreBoulderExtraction : ObjectEntry {
		public override ObjectEntryCategory Category => new(ObjectEntryType.Source, "ItemBrowser:ObjectEntry/OreBoulderExtraction", ObjectID.GoldOreBoulder, 5000);
		...
		public class Provider : ObjectEntryProvider {
			public override void Register(ObjectEntryRegistry registry, List<(ObjectData ObjectData, GameObject Authoring)> allObjects) {
				foreach (var (objectData, _) in allObjects) {
					...
					registry.Register(dropsLootWhenDamagedCD.dropsLoot, 0, new OreBoulderExtraction {
